            ...
            if (message.type === 'audio') {
                ...
                try {
                    ...
                    const mediaUrlResponse = await axios.get(
                        `https://graph.facebook.com/v22.0/${audioId}`, // Use current API version
                    ...
                    const mimeType = message.audio.mime_type || 'audio/ogg; codecs=opus'; // WhatsApp usually sends ogg

                    // Create a temporary file path (optional, can also send buffer directly if API supports)
                    const tempAudioDir = path.join(__dirname, 'temp_audio');
                    if (!fs.existsSync(tempAudioDir)){
                        fs.mkdirSync(tempAudioDir);
                    }
                    const tempFileName = `${audioId}.${mimeType.split('/')[1].split(';')[0]}`; // e.g., audioId.ogg
                    ...
                    const result = await model.generateContent({
                        contents: [{ role: "user", parts: [audioFilePart, {text: "extract text"}] }],
                        generationConfig,
